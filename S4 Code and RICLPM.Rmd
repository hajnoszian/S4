...
ggplot(aes(x = COND, y = KMtot), data = dfI_II) +
  ...
  gghalves::geom_half_point(side = "l", range_scale = .4, alpha = .5) +
    labs(title = "Kama Muta Score by Condition",
       x = "",
       y = "Total KM Score (Out of 16)")
```
...
pp1 <- ggplot(o1, aes(x = KMtot)) +
  geom_bar(aes(y = 100*after_stat(count)/sum(after_stat(count)))) +
  #geom_vline(xintercept = mean(o1$KMtot), color = "red", linetype = "longdash") +
  labs(title = "Score >0 (out of 16) Threshold",
       x = "Number of Kama Muta Days (Out of 14)",
       y = "% of Sample")
```
...
```{r}
#needs the p1-p4 run from above to make these work
grid.arrange(pp1,pp2, pp3,pp4, nrow = 2) #red line indicators are kinda hard to disentangle frankly, so I'm cutting those for now
```
...
ggplot(aes(x = COND, y = Fkm), data = o2) +
  ...
  geom_boxplot(width = .1, outlier.shape = NA) +
  gghalves::geom_half_point(side = "l", range_scale = .4, alpha = .5) +
    labs(title = "Kama Muta by Condition",
       x = "",
...
p1 <-  ggplot(aes(x = day, y = a_Fkm_WPC, color = COND), data = PairwiseF) +
  ...
  scale_x_continuous(n.breaks = 14) +
  ylim(-1,1) +
  labs(title = "Kama Muta Across the Diary Period by Condition",
       x = "",
       y = "WPC KM Score")
...
## Exploratory PA analyses
##### Adding PA Var
Formally should do a full piece by piece variance test here, but for exploratory we'll jump straight to constrained. PA should be pretty stable anyway.
```{r}
measure.m7M <- '
   #####################
  # MEASUREMENT MODEL #
  ...
  # Factor models for PA at 14 waves (Constrained)
  Fpa1 =~ s*pa1_t1 + t*pa2_t1 + u*pa3_t1 + v*pa4_t1 + w*pa5_t1
  Fpa2 =~ s*pa1_t2 + t*pa2_t2 + u*pa3_t2 + v*pa4_t2 + w*pa5_t2
  Fpa3 =~ s*pa1_t3 + t*pa2_t3 + u*pa3_t3 + v*pa4_t3 + w*pa5_t3
  Fpa4 =~ s*pa1_t4 + t*pa2_t4 + u*pa3_t4 + v*pa4_t4 + w*pa5_t4
  Fpa5 =~ s*pa1_t5 + t*pa2_t5 + u*pa3_t5 + v*pa4_t5 + w*pa5_t5
  Fpa6 =~ s*pa1_t6 + t*pa2_t6 + u*pa3_t6 + v*pa4_t6 + w*pa5_t6
  Fpa7 =~ s*pa1_t7 + t*pa2_t7 + u*pa3_t7 + v*pa4_t7 + w*pa5_t7
  Fpa8 =~ s*pa1_t8 + t*pa2_t8 + u*pa3_t8 + v*pa4_t8 + w*pa5_t8
  Fpa9 =~ s*pa1_t9 + t*pa2_t9 + u*pa3_t9 + v*pa4_t9 + w*pa5_t9
  Fpa10 =~ s*pa1_t10 + t*pa2_t10 + u*pa3_t10 + v*pa4_t10 + w*pa5_t10
  Fpa11 =~ s*pa1_t11 + t*pa2_t11 + u*pa3_t11 + v*pa4_t11 + w*pa5_t11
  Fpa12 =~ s*pa1_t12 + t*pa2_t12 + u*pa3_t12 + v*pa4_t12 + w*pa5_t12
  Fpa13 =~ s*pa1_t13 + t*pa2_t13 + u*pa3_t13 + v*pa4_t13 + w*pa5_t13
  Fpa14 =~ s*pa1_t14 + t*pa2_t14 + u*pa3_t14 + v*pa4_t14 + w*pa5_t14
  ...
  pa1_t1 + pa1_t2 + pa1_t3 + pa1_t4 + pa1_t5 + pa1_t6 + pa1_t7 + pa1_t8 + pa1_t9 + pa1_t10 + pa1_t11 + pa1_t12 + pa1_t13 + pa1_t14 ~ x*1
  pa2_t1 + pa2_t2 + pa2_t3 + pa2_t4 + pa2_t5 + pa2_t6 + pa2_t7 + pa2_t8 + pa2_t9 + pa2_t10 + pa2_t11 + pa2_t12 + pa2_t13 + pa2_t14 ~ y*1
  pa3_t1 + pa3_t2 + pa3_t3 + pa3_t4 + pa3_t5 + pa3_t6 + pa3_t7 + pa3_t8 + pa3_t9 + pa3_t10 + pa3_t11 + pa3_t12 + pa3_t13 + pa3_t14 ~ z*1
  pa4_t1 + pa4_t2 + pa4_t3 + pa4_t4 + pa4_t5 + pa4_t6 + pa4_t7 + pa4_t8 + pa4_t9 + pa4_t10 + pa4_t11 + pa4_t12 + pa4_t13 + pa4_t14 ~ aa*1
  pa5_t1 + pa5_t2 + pa5_t3 + pa5_t4 + pa5_t5 + pa5_t6 + pa5_t7 + pa5_t8 + pa5_t9 + pa5_t10 + pa5_t11 + pa5_t12 + pa5_t13 + pa5_t14 ~ bb*1
  
'

measure.m7M.fit <- cfa(measure.m7M,
    data = dfSEM,
    std.lv = F,
    missing = "ML",
    )
#summary(measure.m7M.fit, fit.measures = T, standardized =T, ci = T) #its the fit measures that take forever to cycle
summary(measure.m7M.fit, standardized =T, ci = T) #its the fit measures that take forever to cycle

```

```{r}
#### To make the object with m7 fit measures
o1 <- as.data.frame(predict(measure.m7M.fit)) #woo!
o1$id <- 1:nrow(o1)
colnames(o1)
...
#### To make the actor df: dump df into Actor, then relabel vars
Actor <- df
colnames(Actor)[6:35] <- paste("a", colnames(Actor)[6:35], sep = "_") #adding (back) the actor prefix

#### Isolating Only Needed Vars
Actor <- Actor |> 
  select(pID:day) |> 
  merge(o1, by.x = c("pID", "day"), by.y = c("id", "day"), all.x = T)

colnames(Actor)[6:8] <- paste("a", colnames(Actor)[6:8], sep = "_") #adding (back) the actor prefix
...
### Centering, Previous Day Additions
...
PairwiseFpa |> 
  ...
  mutate(
    ...
    a_Fppr_pm = mean(a_Fppr, na.rm = T),
    p_Fppr_pm = mean(p_Fppr, na.rm = T),
    a_Fpa_pm = mean(a_Fpa, na.rm = T),
    p_Fpa_pm = mean(p_Fpa, na.rm = T)
  ) |> 
  ungroup() |> 
  mutate(
    #Create Grand-Mean Centered Vars (Score - grand mean: How different is given score from sample?)
    ...
    a_Fppr_GMC = scale(a_Fppr, center = T, scale = F),
    p_Fppr_GMC = scale(p_Fppr, center = T, scale = F),
    a_Fpa_GMC = scale(a_Fpa, center = T, scale = F),
    p_Fpa_GMC = scale(p_Fpa, center = T, scale = F),
    #Create Within-Person Centered Vars (Score - person mean score: How different is given score from person's average?)
    a_Fkm_WPC = a_Fkm - a_Fkm_pm,
    p_Fkm_WPC = p_Fkm - p_Fkm_pm,
    a_Fppr_WPC = a_Fppr - a_Fppr_pm,
    p_Fppr_WPC = p_Fppr - p_Fppr_pm,
    a_Fpa_WPC = a_Fpa - a_Fpa_pm,
    p_Fpa_WPC = p_Fpa - p_Fpa_pm,
    #Create Between-Person Centered Vars (GMC - WPC, equation as per Bolger & Laurenceau (2013): How different is person from sample?)
    a_Fkm_BPC = a_Fkm_GMC - a_Fkm_WPC,
    p_Fkm_BPC = p_Fkm_GMC - p_Fkm_WPC,
    a_Fppr_BPC = a_Fppr_GMC - a_Fppr_WPC,
    p_Fppr_BPC = p_Fppr_GMC - p_Fppr_WPC,
    a_Fpa_BPC = a_Fpa_GMC - a_Fpa_WPC,
    p_Fpa_BPC = p_Fpa_GMC - p_Fpa_WPC
    )
```
...
PairwiseFpa |> 
  ...
  mutate(
    ...
    a_Fppr_WPC_prev = a_Fppr_WPC[match(day - 1, day)],
    p_Fppr_WPC_prev = p_Fppr_WPC[match(day - 1, day)],
    a_Fpa_WPC_prev = a_Fpa_WPC[match(day - 1, day)],
    p_Fpa_WPC_prev = p_Fpa_WPC[match(day - 1, day)],
  ) |> 
  ungroup()
```
...
PairwiseFpa |> 
  select(a_Fkm_WPC, p_Fkm_WPC, a_Fppr_WPC, p_Fppr_WPC, a_Fpa_WPC, p_Fpa_WPC) |> 
  psych::pairs.panels(ci = T)

```
Big KM PA correlation. Again, KM seems to be a very positive experience--days I felt more positive than usual also were high KM days
```{r}
cor.test(PairwiseFpa$a_Fkm_WPC, PairwiseF$p_Fkm_WPC, conf.level = .95)
cor.test(PairwiseFpa$a_Fkm_WPC, PairwiseF$a_Fppr_WPC, conf.level = .95)
cor.test(PairwiseFpa$a_Fkm_WPC, PairwiseF$p_Fppr_WPC, conf.level = .95)
cor.test(PairwiseFpa$a_Fppr_WPC, PairwiseF$p_Fppr_WPC, conf.level = .95)

# PA
cor.test(PairwiseFpa$a_Fpa_WPC, PairwiseF$a_Fkm_WPC, conf.level = .95)
cor.test(PairwiseFpa$a_Fpa_WPC, PairwiseF$a_Fppr_WPC, conf.level = .95)
cor.test(PairwiseFpa$a_Fkm_WPC, PairwiseF$p_Fpa_WPC, conf.level = .95)
cor.test(PairwiseFpa$a_Fppr_WPC, PairwiseF$p_Fpa_WPC, conf.level = .95)
cor.test(PairwiseFpa$a_Fpa_WPC, PairwiseF$p_Fpa_WPC, conf.level = .95)
...
```{r}
#Between person fluctuations
PairwiseFpa |> 
  select(a_Fkm_BPC, p_Fkm_BPC, a_Fppr_BPC, p_Fppr_BPC, a_Fpa_BPC, p_Fpa_BPC) |> 
  psych::pairs.panels(ci = T)
```
More big corrs here for PA. People who experience PA more often also experience more KM and PPR. 

```{r}
cor.test(PairwiseFpa$a_Fkm_BPC, PairwiseF$p_Fkm_BPC, conf.level = .95)
cor.test(PairwiseFpa$a_Fkm_BPC, PairwiseF$a_Fppr_BPC, conf.level = .95)
cor.test(PairwiseFpa$a_Fkm_BPC, PairwiseF$p_Fppr_BPC, conf.level = .95)
cor.test(PairwiseFpa$a_Fppr_BPC, PairwiseF$p_Fppr_BPC, conf.level = .95)

# PA
cor.test(PairwiseFpa$a_Fpa_BPC, PairwiseF$a_Fkm_BPC, conf.level = .95)
cor.test(PairwiseFpa$a_Fpa_BPC, PairwiseF$a_Fppr_BPC, conf.level = .95)
cor.test(PairwiseFpa$a_Fkm_BPC, PairwiseF$p_Fpa_BPC, conf.level = .95)
cor.test(PairwiseFpa$a_Fppr_BPC, PairwiseF$p_Fpa_BPC, conf.level = .95)
cor.test(PairwiseFpa$a_Fpa_BPC, PairwiseF$p_Fpa_BPC, conf.level = .95)
```
...
###### KM Outcome
```{r}
m.kmPA <- lmer(a_Fkm_WPC ~ COND + day + #standard controls
               a_Fkm_WPC_prev + p_Fkm_WPC_prev + #actor and partner previous outcome fluctuations
               a_Fppr_WPC_prev + p_Fppr_WPC_prev + #actor and partner previous predictor fluctuations
               a_Fpa_WPC_prev + p_Fpa_WPC_prev + #NEW, PA covariate from previous day  
               a_Fkm_BPC + p_Fkm_BPC + #actor and partner outcome between person, stable trait-like influence
               a_Fppr_BPC + p_Fppr_BPC + #actor and partner predictor between person, stable trait-like influence
               a_Fpa_BPC + p_Fpa_BPC + #NEW, PA stable influence
               (1| cID) + (1|pID), #Random effect structure
             control=lmerControl(optimizer = "bobyqa"),
             data = PairwiseFpa)
summary(m.kmPA) #WOOO! Findings don't change. Effects decrease a SMIDGE but don't meaningfully change. PA is also non sig
confint(m.kmPA)
```
...
```{r}
m.pprPA <- lmer(a_Fppr_WPC ~ COND + day + #standard controls
               a_Fppr_WPC_prev + p_Fppr_WPC_prev + #actor and partner previous outcome fluctuations
               a_Fkm_WPC_prev + p_Fkm_WPC_prev + #actor and partner previous predictor fluctuations
               a_Fpa_WPC_prev + p_Fpa_WPC_prev + #NEW, PA covariate from previous day  
               a_Fppr_BPC + p_Fppr_BPC + #actor and partner outcome between person, stable trait-like influence
             ...
             control=lmerControl(optimizer = "bobyqa"),
             data = PairwiseFpa)
summary(m.pprPA)#WOOO! Findings don't change. Partner effect actually is BIGGER than main results. PA is also non sig
confint(m.pprPA)
```


Results don't change. This is also probably a testament to the RICLPM b/n, w/n disaggregation. Its a robust model that SHOULDNT change much with covariates, which makes me even less concerned with S3 not being able to accomodate. 
